...
class _ToTimestamp(_Transformation):
    ...
    def apply(self, data_frame: DataFrame) -> DataFrame:
        ...
        if format:
            ...
            result_df = data_frame.withColumn(
                target_column,
                F.to_timestamp(F.col(self.column_name).cast(T.StringType())))
        return result_df
...
class _ToTimestampFactory(_TransformationFactory):

    def __init__(self):
        super().__init__('to_timestamp')
    ...
    def from_config(self, spark: SparkSession, column_name: str,
        ...
        return _ToTimestamp(column_name, format, output_column)
