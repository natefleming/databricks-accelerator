...
from honeycomb._constraints._numbers import _MaxFactory, _Max
...
def test_should_raise_when_column_is_not_a_number(spark):
    ...
    validation = Validation(spark, input_df).is_max('name', 10)
...
def test_should_raise_when_invalid_column(spark):
    ...
    validation = Validation(spark, input_df).is_max('invalid', 10)
    with pytest.raises(ValueError, match=r'Missing one or more columns:.*'):
        validation_results = validation.execute()


def test_validate_when_not_max(spark):
    schema = T.StructType([
        T.StructField('name', T.StringType()),
        T.StructField('age', T.IntegerType())
    ])
    ...
    validation = Validation(spark, input_df).is_max('age', 4)
    validation_results = validation.execute()
...
def test_validate_when_not_max(spark):
    ...
    validation = Validation(spark, input_df).is_max('age', 3)
    validation_results = validation.execute()
    ...
    assert validation_error.constraint_name == 'max'
