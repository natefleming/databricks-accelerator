...
from honeycomb._constraints._numbers import _BetweenFactory, _Between
...
def test_factory():
    ...
    config = {'lower_bound': 10, 'upper_bound': 11}
    factory = _BetweenFactory()
    constraint = factory.from_config('mycolumn', config)

    assert type(constraint) == _Between
...
def test_validate_is_between(spark):
    ...
    validation = Validation(spark, input_df).is_between('age', 1, 4)
    validation_results = validation.execute()
...
def test_validate_when_length_not_between(spark):
    ...
    validation = Validation(spark, input_df).is_between('age', 2, 4)
    validation_results = validation.execute()
    ...
    assert validation_error.constraint_name == 'between'
