...
def test_suggest_max_empty(spark):
...
def test_suggest_max(spark):
    ...
    assert suggested_constraint.constraint_name == 'max'
    assert 'value' in suggested_constraint.data
    assert suggested_constraint.data['value'] == 100
